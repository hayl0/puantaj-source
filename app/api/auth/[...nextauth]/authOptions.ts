      ...
      async authorize(credentials) {
        ...
        try {
          ...
          if (user) {
            ...
            if (isPasswordValid) {
              // Check verification
              // Enforce verification only for users created after the feature launch (2030-01-01 - Disabled for now)
              const FEATURE_LAUNCH_DATE = new Date('2030-01-01T00:00:00Z');
          ...
          if (employee && employee.password) {
            ...
            if (isPasswordValid) {
              // Check verification
              // Enforce verification only for employees created after the feature launch (2030-01-01 - Disabled for now)
              const FEATURE_LAUNCH_DATE = new Date('2030-01-01T00:00:00Z');
